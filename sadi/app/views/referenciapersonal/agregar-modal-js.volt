<script type="">
   function fnCrearReferenciaPersonal(){
    let form_ocupado=document.getElementById('frm_crear_referenciapersonal');
    form_ocupado.reset();
     $('#rep_ese_id_crear').text($('#ese_id_ese_actual').text());
     $('#rep_ese_nombre_crear').text($('#ese_nombrecompleto_actual').text()); 

     $('#rep_sep_id').val($('#sep_id').val());
     
    }
    $(function (){
      $('#frm_crear_referenciapersonal').submit(function(event) {
      let $form = $(this);
      a=$form.valid();
      if(a==false){
          return false;
      ...
      $.ajax({
        type: "POST",
        url: url_enviar,
        data: $("#frm_crear_referenciapersonal").serialize(),
        ...
        {   
          ...
          Swal.fire({title:res['titular'],text:res['mensaje'],type:"success"})
            ...
            $('#agregar-referenciapersonal-modal').modal('hide');
...
<div class="modal fade" id="agregar-referenciapersonal-modal" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
   ...
   -->        <div class="modal-header">
              <h5><div id="">
                <i class="mdi mdi-plus"></i>Agregar una referencia personal al estudio No. <span id="rep_ese_id_crear"></span>  "<span id="rep_ese_nombre_crear"></span>"
              </div></h5>
            ...
            <div class="modal-body">
              <!-- //contenido -->
              <form id="frm_crear_referenciapersonal" class="form-vertical mt-1" novalidate method="post">
                <div class="form-group row">
                  <input type="hidden" id="rep_sep_id" name="rep_sep_id" />

                  <div class="col-lg-6">
                    <label class="col-form-label title-busq">Nombre</label>
                    <input name="rep_nombre_crear" id="rep_nombre_crear" type="text" class="form-control input-rounded" required oninput="handleInput(event)"  placeholder="Nombre completo..." maxlength="55" />
                  ...
                  <div class="col-lg-12">
                    ...
                    <textarea id="rep_notas_crear" name="rep_notas_crear" oninput="handleInput(event)" class="form-control-textarea text_area_a" style="min-height:5rem" maxlength="300" placeholder="Notas.."></textarea>

<script>
    function fnGetTodosLosInvestigadores(select_id=0,text_select='Todos'){
		let url="<?php echo $this->url->get('usuario/ajax_get_all_investigadores/') ?>";
		...
		$.ajax({
			...
			if (data.length > 0) {
				$subsnegocio.append(function () {
					...
					options += '<option data-usu_id="'+dat.usu_id+'"  value="' + dat.usu_id + '">' +dat.usu_nombre+'</option>';
					});

<script type="text/javascript">
    $(document).ready((event)=>{
        $('#com_fechainicial').val(moment().format('YYYY-MM-DD'));
        $('#com_fechafinal').val(moment().format('YYYY-MM-DD'));
    });

  function principal(){

    let fecha_inicial= $('#com_fechainicial').val();
    let fecha_final=$('#com_fechafinal').val();
    let validacion=true;
    ...
    if(validacion)
    {
      ...
      urlreloadprincipal="<?php echo $this->url->get('reporte/comentarioese_tabla/') ?>";
...
<div class="row">
  <div class="col-6">
          <h4 class="header-title header-title-crm">Comentarios</h4>
  </div>
  <div class="col-6">
      ...
      {% if acceso.verificar(47,rol_id)==1 %}
        <form id="form_estatus_estudio"  class="form-vertical col-md-12 row">
            ...
            <label class="col-form-label title-busq">Usuario</label>
                <select name="usu_id" id="usu_id" data-toggle="select2" class="form-control select2-multiple">
                    <option value="-1">Todos</option>
                    {% for usu in usuarioselect %}
                    <option value="{{usu.usu_id}}" {% if indexusu== usu.usu_id%} selected {% endif %}>{{usu.nombre}} </option>
                    {% endfor %}
                </select>
            </div>
            <div class="col-lg-6 " id="fecha_alta_div" >
             
                <label class="col-form-label  title-busq">Fecha de registro<i class="mdi mdi-upload"></i></label>
                <div>
                    <div class="input-group" id="">
                        <label class="col-form-label  title-busq">Desde</label>
                        <input type="date" id="com_fechainicial" name="com_fechainicial" class="form-control bar-left" placeholder="Desde" />
                        <label class="col-form-label  title-busq">Hasta</label>
                        <input type="date" id="com_fechafinal" name="com_fechafinal" class="form-control bar-right" placeholder="Hasta" />
                    </div>
                
                </div>
            </div>
            <div class="col-lg-6" id="">
            <label class="col-form-label title-busq">Tipo de comentario</label>
                <select name="com_regresoautoriza" id="com_regresoautoriza" data-toggle="select2" class="form-control select2-multiple">
                    <option value="-1">Todos</option>
                    <option value="1" selected>Regresado de autorización</option>
                </select>
            </div>
          ...
          <div class="col-lg-1 col-3  text-right mt-4">
            <div class="form-group">
              {{ link_to('reporte/comentarioese_index', '<i class="mdi mdi-eraser white" title="Limpiar búsqueda"></i>',"class": "btn-dark btn-rounded btn btn-limpiar") }}
            </div>
          </div>
        </form>
      {% endif %}

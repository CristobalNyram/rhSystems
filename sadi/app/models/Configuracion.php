...
class Configuracion extends Model
{
    ...
    public $dafault_id_envio_correos=15;
    public $dafault_value_envio_correos=1;
    public $default_id_captcha=17;
    ...
    public function init_configuracion_lime_api(){
        $answer = [
    
            "link_remote_control" => null,
            "pas_remote_control" => null,
            "user_remote_control" => null,
        ];
    
        $link_remote_control = Configuracion::findFirst(11);
        $user_remote_control = Configuracion::findFirst(12);
        $pas_remote_control = Configuracion::findFirst(13);

    
        if ($link_remote_control) {
            $answer["link_remote_control"] = $link_remote_control->cof_valor;
        }
    
        if ($pas_remote_control) {
            $answer["pas_remote_control"] = $pas_remote_control->cof_valor;
        }
    
        if ($user_remote_control) {
            $answer["user_remote_control"] = $user_remote_control->cof_valor;
        }
    ...
    public function get_estatuscaptcha(){
        $config = Configuracion::findFirstBycof_id($this->default_id_captcha);
        if($config->cof_valor==1){
            return true;
        }else{
            return false;
        }

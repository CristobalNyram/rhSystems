...
class Usuario extends Model
{
	...
	public function getEstatusNombre($valor=0)
	{
		if ($valor == 1) 
		{
			return 'Baja';
	...
	public function NuevoRegistro($data,$id)
	{
		...
		if (isset($data['emp_id'])) {
			$usuario->emp_id = $data['emp_id'];
		}
		$usuario->est_id=$data['est_id'];
		$usuario->mun_id=$data['mun_id'];
		$usuario->usu_adicional=$data['usu_adicional'];
	...
	public function actualizarPerfil($data){
		...
		$this->usu_adicional=$data['usu_adicional'];

		$empresa=null;
		if (isset($data["emp_ideditar"]) && $data["emp_ideditar"] != -1) {

...
//estatus
    ...
    public function NuevosCampos()
    {
        ...
        $password->setLabel('Contraseña');
        $password->setAttributes(array('placeholder' => "Contraseña", "class"=>"form-control input-rounded"));
        $password->addValidators(array(
            new PresenceOf(array(
                'message' => 'Contraseña requerida'
            ))
        ...
        $nombre->setLabel("Nombre");
        $nombre->setFilters(array('striptags', 'string'));
        $nombre->setAttributes(array('maxlength' => 150, 'placeholder' => "Nombre(s)", "class"=>"form-control input-rounded","onkeyup"=>"javascript:this.value=this.value.toUpperCase();"));
        $nombre->addValidators(array(
            new PresenceOf(array(
                'message' => 'Nombre obligatorio'
            ))
        ...
        $apellidop->setFilters(array('striptags', 'string'));
        $apellidop->setAttributes(array('maxlength' => 150, 'placeholder' => "Primer apellido", "class"=>"form-control input-rounded","onkeyup"=>"javascript:this.value=this.value.toUpperCase();"));
        $apellidop->addValidators(array(
            new PresenceOf(array(
                'message' => 'Apellido obligatorio'
            ))
        ...
        $apellidom->setFilters(array('striptags', 'string'));
        $apellidom->setAttributes(array('maxlength' => 150, 'placeholder' => "Segundo apellido", "class"=>"form-control input-rounded","onkeyup"=>"javascript:this.value=this.value.toUpperCase();"));
        $apellidom->addValidators(array(
            new PresenceOf(array(
                'message' => 'Apellido obligatorio'
            ))
        ...
        $correo->setLabel('Correo electrónico');
        $correo->setFilters(array('striptags', 'email'));
        $correo->setAttributes(array('placeholder' => "Correo electrónico", "class"=>"form-control input-rounded"));
        $this->add($correo);

        $estatus = new Select('usu_estatus');
        $estatus->setLabel('Estatus');
        $estatus->setAttributes(array("class"=>"form-control select2-single bar-right"));
        $estatus->setOptions(array('2'=>'Alta','1'=>'Baja'));
        $this->add($estatus);

        $telefono = new Text("usu_telefono");
        $telefono->setLabel("Teléfono");
        // $telefono->setFilters(array('striptags', 'string'));
        $telefono->setAttributes(array('maxlength' => 15, 'placeholder' => "Teléfono", "class"=>"form-control input-rounded","onkeyup"=>"javascript:this.value=this.value.toUpperCase();"));
        $this->add($telefono);

        $nombre = new Submit("enviar");
        $nombre->setLabel("");
        $nombre->setAttributes(array('value' => "Guardar", 'class' =>"btn-dark btn-rounded btn btn-buscar submit"));
        $this->add($nombre);
    ...
    public function EditarCampos()
    {
        ...
        $id->setLabel('ID');
        $id->setFilters(array('striptags', 'string'));
        $id->setAttributes(array('maxlength' => 150, "style"=>"cursor: not-allowed;", 'placeholder' => "Clave", "class"=>"form-control input-rounded-disabled",'readonly' => "true"));
        $this->add($id); 
        ...
        $nombre = new Submit("enviar");
        $nombre->setLabel("");
        $nombre->setAttributes(array('value' => "Actualizar", 'class' =>"btn-dark btn-rounded btn btn-buscar submit"));
        $this->add($nombre);

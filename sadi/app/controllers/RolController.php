...
class RolController extends ControllerBase
{
    public function initialize()
    {
        ...
        // if(!$rol->verificar(2,$auth['rol_id'])) //el número en la funcion es el correspondiente a la bdd
        // {
    ...
    public function editarAction($id=0)
    {
        ...
        if (!$this->request->isPost()) 
        {
            ...
            $this->tag->setDefault('rol_nivel',$rol->rol_nivel);
            $this->tag->setDefault('rol_traaprobar',$rol->rol_traaprobar);
            $this->tag->setDefault('rol_trasolicitar',$rol->rol_trasolicitar);
    ...
    public function permisoAction($id='')
    {
        ...
        if (!$this->request->isAjax()) 
        {
            ...
            $relpuemenu=$relpuemenu->columns(array('r.rrm_id','r.rrm_estatus','m.men_id','m.men_titulo','m.men_estatus','m.men_padre', 'm.men_grupo', 'p.rol_nombre','p.rol_estatus','p.rol_id','r.rrm_id as datos'))
        ...
        else
        {
            ...
            $data = $this->request->getPost();
            $relpuemenu=new Builder();
            $relpuemenu=$relpuemenu->columns(array('r.rrm_id','r.rrm_estatus','m.men_titulo','m.men_estatus', 'm.men_grupo', 'p.rol_nombre','p.rol_estatus','p.rol_id'))
                ->addFrom('Relrolmenu','r')
                ->join('Rol','p.rol_id=r.rol_id','p')
                ->join('Menu','r.men_id=m.men_id','m')
    ...
    public function ajax_rolesAction()
    {
        $result = [];
         $auth = $this->session->get('auth');
        // $id_usuario = $auth['id'];
        ...
        $subs = Rol::find(array(
            'rol_estatus<=2  and rol_nivel >'.$auth['rol_nivel'],"order"=>"rol_nombre"
            ));
        if ($subs) {
            $result = $subs->toArray();
        }

     ...
     */
      ...
      public function guardarAction()
      {
            ...
            if($this->request->isAjax())
            {
                  ...
                  $cuestionario = new Rescuetres();
                  $cuestionario->cal_id=$calificacion->getCalificacion(3,$data['54']);
      ...
      public function respuestaAction(){
            ...
            $folioinicial=$ini;
            $foliofinal=$fin;
            $condicion="f.fol_estatus=2 and fot_id>=".$folioinicial." and fot_id<=".$foliofinal;




            $folios=Foliocuetres::query()
                  ->columns('fot_id,emp_id')
                  ->join('Folio','f.fol_id=Foliocuetres.fot_id','f')
                  ->where($condicion)
                  ->execute();
            ...
            for ($i=0; $i < count($folios); $i++) {
                  ...
                  for ($j=0; $j < count($respuesta); $j++) {
                        $objPHPExcel->getActiveSheet()->SetCellValue("A".$posicion, $respuesta[$j]->fot_id);
      ...
      public function revisarfolioAction($folio)
      {
            ...
            if($folioval)
            {
                $foliocue=Foliocuetres::findFirstByfot_id($folio);
                if($foliocue)
                {
                    $answer[0]=-1;

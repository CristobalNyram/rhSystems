...
class CursoabiertoController extends ControllerBase
{
    public function initialize()
    {
        $this->tag->setTitle('Curso abierto');
        parent::initialize();
        $rol = new Rol();
    ...
    public function tablaAction()
    {
        ...
        $curso = Cursootorgado::find(array(
            "cuo_estatus<=2 and cuo_estatus>=0 and cuo_tipo=2"
        ));
        ...
        $databit['bit_descripcion']= "Consultó catálogo de cursos abiertos";
    ...
    public function participantesAction($id)
    {
        ...
        $auth = $this->session->get('auth');
        $bitacora= new Bitacora();
        $databit['bit_descripcion']= "Consultó participantes de curso abierto ".$cuo[0]->cuo_clave;
        $databit['usu_id']=$auth['id'];
    ...
    public function reportedc3Action($id)
    {
        ...
        if($part->cen_id!=null)
        {
            ...
            //INICIA FOOTER
                ...
                $hisdescarga->NuevoRegistrodc3($historial,$auth['id'],2);

                $auth = $this->session->get('auth');
                $bitacora= new Bitacora();
                $databit['bit_descripcion']= "Descargó un documento DC3 de manera individual (curso abierto)";
                $databit['usu_id']=$auth['id'];
                $databit['bit_tablaid']=$id;
                $bitacora->NuevoRegistro($databit);
            //TERMINA FOOTER
    ...
    public function formularioAction($clave="")
    {
        ...
        if($this->request->isPost())
        {
            ...
            { 
                if($clave==""){
                    $clave=$res;
                    $databit['bit_descripcion']= "Creó un curso abierto";
                }else{
                    $databit['bit_descripcion']= "Editó un curso abierto con id ".$clave;
                }
                ...
                $this->response->redirect('cursoabierto/index/');
                $this->view->disable();
                return;
            }
            else
        ...
        if($clave!="")
        {
            ...
            if(!$curso)
            {
                ...
                return $this->forward("cursoabierto/index");
            }
            $clases=array(
...
/*    public function eliminarAction($id)
    ...
    public function participantenuevoAction()
    {
        if($this->request->isPost())
        {
            ...
            if($this->request->hasFiles() == true){
                        ...
                        if($res>0){
                            if($data['tra_curp']!= ' ')
                            {
                                ...
                                else
                                {
                                    ...
                                    $auth = $this->session->get('auth');
                                    $bitacora= new Bitacora();
                                    $databit['bit_descripcion']= "Agregó un participante ".$trabajador->tra_curp." masivamente al curso abierto con id ".$datap['cuo_id'];
                                    $databit['usu_id']=$auth['id'];
                                    $databit['bit_tablaid']=$res;
                                    $bitacora->NuevoRegistro($databit);
                            ...
                            else
                            {
                                ...
                                $auth = $this->session->get('auth');
                                $bitacora= new Bitacora();
                                $databit['bit_descripcion']= "Agregó un participante ".$curpvaciaidtra." masivamente al curso abierto con id ".$datap['cuo_id'];
                                $databit['usu_id']=$auth['id'];
                                $databit['bit_tablaid']=$res;
                                $bitacora->NuevoRegistro($databit);
                            }
                ...
                $this->response->redirect('cursoabierto/participantes/'.$data['id_cuo']);
                $this->view->disable();
                return;
            }
        }
    ...
    public function dc3masaAction()
    {
        ...
        for($i=1;$i<=$var;$i++){
            ...
            if($part->cen_id!=null)
            {
                ...
                ->columns(array('tra_nombre','tra_primerapellido','tra_segundoapellido','tra_curp','p.tra_puesto','cur_nombre','cuo_horas','cuo_diploma',
                    ...
                    'p.par_id','p.rep_idlegal','p.rep_idtra','p.par_foliodc3','DATE_FORMAT(par_fechadc3, "%d-%m-%Y") as fechadc3', 'par_fechadc3 as fecha_dc3doc'))
                ->addFrom('Participante','p')
                ->join('Trabajador','p.tra_id=t.tra_id','t')
                ->join('Cursootorgado','co.cuo_id=p.cuo_id','co')
                ->join('Curso','c.cur_id=co.cur_id','c')
        ...
        $auth = $this->session->get('auth');
        $bitacora= new Bitacora();
        $databit['bit_descripcion']= "Descargó documentos DC3 de manera masiva de curso abierto";
        $databit['usu_id']=$auth['id'];
        $databit['bit_tablaid']=0;
        $bitacora->NuevoRegistro($databit);
    ...
    public function eliminarmasaAction()
    {
        ...
        $auth = $this->session->get('auth');
        $bitacora= new Bitacora();
        $databit['bit_descripcion']= "Eliminó ".$eliminados." participantes de curso abierto";
        $databit['usu_id']=$auth['id'];
        $databit['bit_tablaid']=0;
        $bitacora->NuevoRegistro($databit);
    ...
    public function reportediplomaAction($id)
    {
        ...
        $filename = $dir.'qr.png';
            ...
            $auth = $this->session->get('auth');
            $bitacora= new Bitacora();
            $databit['bit_descripcion']= "Descargó un documento Diploma de manera individual de curso abierto";
            $databit['usu_id']=$auth['id'];
            $databit['bit_tablaid']=$id;
            $bitacora->NuevoRegistro($databit);
        }
        ...
        $historial=$participante[0];
        $hisdescarga= new Historialdescarga();
        $hisdescarga->NuevoRegistrodiploma($historial,$auth['id'],2);
            // $pdf->Output();
        $this->view->pdf=$pdf;
    ...
    public function diplomamasaAction()
    {
        ...
        $auth = $this->session->get('auth');
        $bitacora= new Bitacora();
        $databit['bit_descripcion']= "Descargó documentos Diploma de manera masiva de curso abierto";
        $databit['usu_id']=$auth['id'];
        $databit['bit_tablaid']=0;
        $bitacora->NuevoRegistro($databit);

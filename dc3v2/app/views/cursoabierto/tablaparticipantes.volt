				...
				<tr>
					...
					<td width="7%">
						{% if admindirector!=null %}
							{% if dc3==1 and centro!=""%}
								{{ link_to("cursoabierto/reportedc3/"~par.par_id, '<i class="fa fa-file"></i>', "class": "btn","title":"Descargar DC3",'target':'_blank') }}
							{% endif %}
							{% if diploma==1 %}
								{{ link_to("cursoabierto/reportediploma/"~par.par_id, '<i class="fa fa-trophy"></i>', "class": "btn","title":"Descargar Diploma",'target':'_blank') }}
							{% endif %}
						{% endif %}
						...
						<a type="button" class="btn" title='Eliminar' onclick="fnelim('{{par.par_id}}','{{ nombre }}')"><i class="fa fa-trash-o"></i></a>
						{% endif %}

...
<script type="text/javascript">
  ...
  function GetSelected() 
  {
    ...
    var urlmasa="<?php echo $this->url->get('cursolinea/dc3masa/') ?>";
  ...
  function GetSelectedDiploma() {
    ...
    jsondatos.arreglo=arreglo;
    // console.log(jsondatos);
    var urlmasa="<?php echo $this->url->get('cursolinea/diplomamasa/') ?>";
    // $form.find("button").prop("disabled", true);
    $.ajax({
      type: "POST",
      url: urlmasa,
  ...
  function GetSelectedEliminar() {
    ...
    { 
      ...
      jsondatos.arreglo=arreglo;
      // console.log(jsondatos);
      var urlmasa="<?php echo $this->url->get('cursolinea/eliminarmasa/') ?>";
      // $form.find("button").prop("disabled", true);
      $.ajax({
        type: "POST",
        url: urlmasa,
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    url="<?php echo $this->url->get('cursolinea/tablaparticipantes/"+id_curso+"') ?>";
  ...
  $(function (){
    ...
    $("#frm_asignaremp").submit(function(event) 
    {
      /* Act on the event */
      var $form = $(this);
      var urlcrear="<?php echo $this->url->get('cursolinea/asignarempresa/') ?>";
      $form.find("button").prop("disabled", true);
      $.ajax({
        type: "POST",
        url: urlcrear,
    ...
    $("#frm_asignarcen").submit(function(event) 
    {
      ...
      var $form = $(this);
      var urlcrear="<?php echo $this->url->get('cursolinea/asignarcentro/') ?>";
      $form.find("button").prop("disabled", true);
      $.ajax({
        type: "POST",
        url: urlcrear,
    ...
    $("#frm_fechaexamen").submit(function(event) 
    {
    /* Act on the event */
    var $form = $(this);
    var urlpag="<?php echo $this->url->get('cursolinea/fechaexamen/') ?>";
    $form.find("button").prop("disabled", true);
    $.ajax({
      type: "POST",
      url: urlpag+$participantefecha,
      data: $("#frm_fechaexamen").serialize(),
      success: function(res)
      {
        if(res[0]<=0)
        {
        ...
        {
          // cargarlista();
          alertify.alert("Éxito","Fecha asignada correctamente.", function(){ 
            location.reload();
            // document.getElementById("recibolistado").innerHTML="";

            // // reciboListado = document.getElementById('recibolistado');
            // tablaprincipal();
            // reloadrecibo(res[2]);
            // $('#asigfechaexamen-modal').modal('hide')
          });
  ...
  function fnelim(are,clave)
  {
    ...
    var urlindexare="<?php echo $this->url->get('cursolinea/participantes/') ?>";
  ...
  function fnasigfechaexamen(id)
    {
        $participantefecha=id;
        document.getElementById("participantefecha").value = id;
...
<div id="cargarparticipante" class="modal fade" role="dialog">
  <div class="modal-dialog">
    {{ form('cursolinea/participantenuevo', 'id': 'participantesform', 'enctype': 'multipart/form-data') }}
...
<div class="modal fade" id="asigfechaexamen-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
  <div class="modal-dialog">
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          ...
          <div class="x_content">
            <br />
            <form id="frm_fechaexamen" data-parsley-validate class="form-horizontal form-label-left captura">
              <!-- <input type="hidden" id="rep_idedit" name="rep_idedit" /> -->
              <input type="hidden" id="participantefecha" name="participantefecha" />
              
              <div class="form-group">
                <label class="control-label col-md-4 col-sm-4 col-xs-12">Fecha de examen
                </label>
                <div class="col-md-7 col-sm-7 col-xs-12">
                  <input type="date" id="fecha_examen" name="fecha_examen" class="sub form-control col-md-7 col-xs-12">
                </div>
              </div>
              <hr>
              <div class="ln_solid">
              ...
              <div class="form-group">
                <div class="col-md-3 col-sm-3 col-xs-12 col-md-offset-6">
                  <button type="submit" class="btn btn-block add btn-btnempresa">Aceptar</button>
                </div>
                <div class="col-md-3 col-sm-3 col-xs-12">
                  <button class="btn btn-block cancelar" class="close" data-dismiss="modal" aria-label="Close">Cancelar</button>
                </div>

...
class Menu extends Model
{
	...
	public function NuevoRegistro($data)
	{
		...
		if ($menu->save() == false){
			...
			$listapuesto = Puesto::find(array(
            "pue_estatus<=2 and pue_estatus>=1"
            ));
			// $listamenu=Menu::all();
			for ($i=0; $i < count($listapuesto); $i++)
			{
				$relpuemenu=new Relpuestomenu();
				$relpuemenu->men_id=$men_id;
				$relpuemenu->pue_id=$listapuesto[$i]->pue_id;
				// $relpuemenu->pue_id=$pue_id;
				$relpuemenu->rpm_estatus=0;
				if(!$relpuemenu->save())
				{
					return false;
				}

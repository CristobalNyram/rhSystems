    ...
    public function mainAction()
    {
        ...
        $io->on('connection', function ($socket) use ($io) {
            ...
            $socket->on('join', function ($user) use ($socket) {
                ...
                ChatTask::log($user['usu_nombre'] . ' se ha conectado.');

                // Asignar usuario
                if (isset(ChatTask::$users[$user['usu_id']])) {
                    ChatTask::$users[$user['usu_id']]['sockets'][] = $socket->id;
                } else {
                    ChatTask::$users[$user['usu_id']] = $user;
                    ChatTask::$users[$user['usu_id']]['sockets'][] = $socket->id;
                }
            });
            ...
            $socket->on('message', function ($data) use ($io, $socket) {
                ...
                if ( ! $conversation) {
                    ChatTask::log('No se pudo guardar el mensaje');
                }
                ...
                if (is_null($conversation->pro_id))
                {
                    ...
                    ChatTask::log("Dest: " . $recipient_id);
                ...
                if (count($recipients) > 0) {
                    ...
                    foreach ($recipients as $recipient_id) {
                        ...
                        if (isset(ChatTask::$users[$recipient_id])) {

                            ChatTask::log("Sockets: " . count(ChatTask::$users[$recipient_id]['sockets']));

                            // Enviar en todos los chats conectados
                            foreach (ChatTask::$users[$recipient_id]['sockets'] as $socket_id)
                            {
            ...
            $socket->on('disconnect', function () use ($socket) {
                ...
                $deleteId = null;
                foreach (ChatTask::$users as $user_id => $user) {
                ...
                if ($deleteId) {
                    $name = ChatTask::$users[$deleteId]['usu_nombre'];
                    unset(ChatTask::$users[$deleteId]);
                    ChatTask::log($name . ' se ha desconectado');
                }
            });
        });

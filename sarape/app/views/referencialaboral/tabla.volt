...
{% set cincuentaydos  = acceso.verificar(42,rol_id) %}
...
{% set total_rel = page|length %}
    ...
    {% if loop.first %}
		<div class="table-responsive m-3 col-12" id="lista-agrupada" style="padding-right: 20px; padding-left: 20px;">
		    <table id="dato_referencialaboral_table" class="table table-striped table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
		        <thead class="thead-light-crm">
		            <tr>	
						<th>ID</th>
						<th>Orden</th>
		                <th>Empresa -candidato</th>
		                <th>Domicilio -candidato</th>
		                <th>Jefe directo inmediato -candidato</th>
		                <th>Teléfono -candidato</th>
		                <th>Puesto inicial  -candidato</th>
		                <th>Puesto final -candidato</th>
						<th>Entrada  -candidato</th>
		                <th>Salida -candidato</th>
		                <th>Sueldo incial -candidato</th>
		                <th>Sueldo final  -candidato</th>
		                <th>Motivo de separación -candidato</th>
						<th>Incapacidad -candidato</th>
						<th>Demanda -candidato</th>
						<th>Recomendable -candidato</th>
						{% if cuarenta==1 %}
						<th>Empresa -RH</th>
		                <th>Domicilio -RH</th>
		        ...
		        {% endif %}
		            ...
		            <tr>
						...
						<td>
							{{  getSiNoObject.getSiONo(rel.rel_adicciones)}}
						...
						<td>
							...
							<a data-toggle="modal" title="Editar" type="button" class="" data-container="body" data-toggle="popover" role="button" data-target="#editar-referencialaboral-modal" onclick="fnEditarReferenciaLaboral(
								'{{rel.rel_id}}','{{  cuarenta}}','{{ cuarentayuno }}')" >
								<i class="mdi mdi-pencil mdi-18px btn-icon"></i>
							</a>
							{% if cincuentaydos==1 %}
							<a title="Eliminar" type="button" class="" data-container="body" data-toggle="popover" role="button" onclick="fnElimnaReferenciaLaboralGeneral('{{ rel.rel_id }}',fnCargarTablaDatoReferenciaLaboral)">
							...
							{% if noventaycuatro==1 %}
									...
									<a data-toggle="modal" title="Cambiar orden arriba" 
									type="button" class="" data-container="body"
									 data-toggle="popover" role="button" 
									  onclick="fnArribaOrdenCambiar('{{rel.rel_id}}',fnCargarTablaDatoReferenciaLaboral)"
									  style="display: {{ btn_arriba_display }};"	
									   >
									...
									<a data-toggle="modal" title="Cambiar orden abajo" 
									type="button" class=""
									data-container="body" data-toggle="popover" 
									role="button"  onclick="fnAbajoOrdenCambiar('{{rel.rel_id}}',fnCargarTablaDatoReferenciaLaboral)"
									style="display: {{ btn_abajo_display }};"	
									>

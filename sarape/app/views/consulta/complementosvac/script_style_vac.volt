...
<script>
  $(function() {
    // Hacer elementos con la clase "grupo-filtro" arrastrables
  /*  $(".grupo-filtro").draggable({
      // Configura las opciones según tus necesidades
      containment: "#container-menu-options", // Limita el movimiento al contenedor principal si es necesario
      cursor: "move", // Cambia el cursor para indicar que se pueden arrastrar
      
      
    });*/
     // Hacer elementos con la clase "grupo-filtro" ordenables
    $("#container-menu-options").sortable({
      axis: "y", // Permite reordenar verticalmente
      tolerance: "intersect", // Permite ajustar automáticamente cuando se superponen
 //     cursor: "move", // Cambia el cursor para indicar que se pueden reorganizar
     cursor: "grabbing", // Cambia el cursor mientras se arrastra
      stop: function(event, ui) {
      // Obtener todas las posiciones de los elementos "grupo-filtro"
      var posiciones = {};
      $(".grupo-filtro").each(function(index, element) {
        posiciones[element.id] = $(element).index();
        console.log(index,element);
      });

      // Guardar las posiciones en localStorage
      localStorage.setItem("posicionesGrupoFiltro", JSON.stringify(posiciones));
    }

    
  });

  // Recuperar las posiciones desde localStorage
  var posicionesGuardadas = localStorage.getItem("posicionesGrupoFiltro");
  if (posicionesGuardadas) {
    var posiciones = JSON.parse(posicionesGuardadas);
    
    // Restaurar la posición de los elementos "grupo-filtro" según las posiciones guardadas
    $.each(posiciones, function(id, posicion) {
              console.log(id,posicion);

    //  $("#" + id).appendTo("#container-menu-options").css({ order: posicion });
    });
  }
    $(".busqueda-item").draggable({
      // Configura las opciones según tus necesidades
      containment: "#wrapper", // Limita el movimiento al contenedor principal si es necesario
      cursor: "move", // Cambia el cursor para indicar que se pueden arrastrar
      
      
    });
      // Hacer el elemento "busqueda-filtros" soltable

<style>
  #datos-info-mandar_garantia_exc_general{
    display: flex;
    justify-content: space-around;
  }
  #datos-info-mandar_garantia_exc_general .badge{
    font-size: 1rem;
    padding: 1rem;
...
<script>
    ...
    let EXC_ID_MANDAR_GARANTIA=0; 
    let CALLLBACK_MANDAR_GARANTIA=0; 
    let CALLLBACK_MANDAR_GARANTIA_TABLA_VISTA_PRINCIPAL=0; 

    let VAC_ID_MANDAR_GARANTIA=0; 

    function fnMandarAGarantiaExc(exc_id=0,vac_id=0,callback=0,callback_principal_vista=0){
        EXC_ID_MANDAR_GARANTIA=exc_id;
        VAC_ID_MANDAR_GARANTIA=vac_id;
        CALLLBACK_MANDAR_GARANTIA=callback;
        CALLLBACK_MANDAR_GARANTIA_TABLA_VISTA_PRINCIPAL=callback_principal_vista;
        $('#form_mandar_garantia_exc_general')[0].reset(); // Reinicia el formulario
        fnGetDetalleExc(exc_id)
                          .then(function(res) {
                              ...
                              let data=res.data;
                                ...
                                $("#mandar_garantia_exc_general-titulo").html(mensaje);
                                $("#exc_id-mandar_garantia_exc_general").val(exc_id);
                                $("#vac_id-mandar_garantia_exc_general").val(data.vac_id);
              ...
              $.ajax({
                  type: "POST",
                  url: url+VAC_ID_MANDAR_GARANTIA,
                  success: function(res)
                  {
                   ...
                   $(`#datos-title-mandar_garantia_exc_general`).html(`<h5 class="text-center">VACANTE: ${mensaje_vac}<h5>`);


                  // $('#nota-mandar_garantia_exc_general').html(`Nota: No puedes maandar mas de un limite permitido de garantía ${data.vac_numero}`);

                    $('#datos-info-mandar_garantia_exc_general').html(`
                  ...
                  `;
                    ...
                    $("#progressbar-mandar_garantia_exc_general").html(progress_bar);
     ...
     $(document).ready(()=>{
		...
		$("#form_mandar_garantia_exc_general").submit(function(event) 
    {
            event.preventDefault();
		        let $form = $(this);  
            let urled="<?php echo $this->url->get('expedientecan/mandar_garantia/') ?>";
            $form.find("button").prop("disabled", true);
            $.ajax({
              type: "POST",
              url: urled+EXC_ID_MANDAR_GARANTIA,
              data: $form.serialize(),
              success: function(res)
              { 
                switch (res['estado']) {
                      ...
                      if(CALLLBACK_MANDAR_GARANTIA=="0"){
                        location.reload();  
                      }else{
                          CALLLBACK_MANDAR_GARANTIA(res["vac_id"]);
                          if(CALLLBACK_MANDAR_GARANTIA_TABLA_VISTA_PRINCIPAL!="0"){                             
                              if( res['estatus_callback_tabla_principal']=="2"){
                                CALLLBACK_MANDAR_GARANTIA_TABLA_VISTA_PRINCIPAL();
                              }
                          } 
                      }
                        //fnCargarTablaCitas(res['vac_id'])
                        $("#mandar_garantia_exc_general-modal").modal("hide");
...
{{  modal.crear("Mandar a garantia expediente  <span id='mandar_garantia_exc_general-titulo'><span>", "form_mandar_garantia_exc_general","mandar_garantia_exc_general-modal",
[
{"value":"<div class='col-12 mt-1 mb-1' id='datos-title-mandar_garantia_exc_general'></div>","tipo":"html"},
{"value":"<div class='col-12 mt-1 mb-1' id='progressbar-mandar_garantia_exc_general'></div>","tipo":"html"},
{"value":"<div class='col-12 mt-1 mb-1' id='datos-info-mandar_garantia_exc_general'></div>","tipo":"html"},
 {"tamanio":"0","leyenda":"","id":"vac_id-mandar_garantia_exc_general","name":"vac_id","tipo":"hidden","required":"","funcion":'onchange=""',"clase":"","value":"0"},
 {"tamanio":"0","leyenda":"","id":"exc_id-mandar_garantia_exc_general","name":"exc_id","tipo":"hidden","required":"","funcion":'onchange=""',"clase":"","value":"0"},
 {"tamanio":"12","leyenda":"MOTIVO","id":"gar_observacion-mandar_garantia_exc_general","name":"gar_observacion","tipo":"textarea","required":"","complemento":'style="min-height:100px"'},
 {"value":"<div class='col-12 mt-1 mb-1' id='nota-mandar_garantia_exc_general'></div>","tipo":"html"}
 ],

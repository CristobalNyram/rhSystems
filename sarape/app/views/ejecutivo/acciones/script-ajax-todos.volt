 ...
 * @return {void}
 */
function fnejecutivos_adaptable(input_select_eje, eje_select_id = 0,excluir_eje=0) {
  ...
  if (!ejecutivo || !$select_usado) {
	...
	let url_enviar = '<?php echo $this->url->get("ejecutivo/ajax_get_all_ejecutivos/") ?>' + `${ejecutivo}/${excluir_eje}`;
  	$select_usado.empty();
  $.ajax({
    type: "POST",
    url: url_enviar,

<script type="text/javascript">
  $(function (){
      $("#frm_crearnegocio").submit(function(event) 
      {
        /* Act on the event */
        var $form = $(this);
        var urled="<?php echo $this->url->get('ocupacion/nuevo/') ?>";
        a=$form.valid();
        if(a==false){
            return false;
        }
      ...
      success: function(res)
      {
        if(res[0]<=0)
          ...
          {
          Swal.fire({title:'Éxito',text:'Ocupación creado correctamente.',type:"success"})
            .then((value) => {
              location.reload();
            });
        }
    ...
    {
        var urleliminarempre="<?php echo $this->url->get('ocupacion/eliminar/') ?>";
        mensaje="¿Está seguro que desea eliminar la ocupación "+nombre_negocio+" ?";
    ...
    function fneditnegocio(id,nombre)
    {
      ...
      var urlfned="<?php echo $this->url->get('ocupacion/buseditar/') ?>"; //trabajador
      $edempresa=id;
      $.ajax(
      {
        type: "POST",
        ...
        success: function(res)
        {
          ...
          {
            $("#ocu_ideditar").val(res[1].ocu_id);
            $("#ocu_nombreeditar").val(res[1].ocu_nombre);
          }
        }
      });
    }
  ...
  $(function (){
    $("#frm_editarnegocio").submit(function(event) 
    {
      /* Act on the event */
      var $form = $(this);
      var urledtra="<?php echo $this->url->get('ocupacion/editar/') ?>";
      $form.find("button").prop("disabled", true);
      $.ajax({
        type: "POST",
        ...
        success: function(res)
        {
          ...
          if(res[0]==1)
          {
            alertify.alert("Éxito","Ocupación editada correctamente.", function(){
              location.reload();
            });
...
<div class="row">
  <div class="col-sm-6">
    <h4 class="header-title header-title-crm">Ocupación</h4>
  </div>
  <div class="col-sm-5">
    <div class="text-right curso" style="margin-top: 35px; font-weight: 600">
      <a href="#">NUEVA OCUPACIÓN</a>
    </div>
  </div>
  <div class="col-sm-1">
  <div class="text-left">
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="" id="exampleModalLabel">Crear ocupación</h5>
      ...
      <div class="modal-body">
        <form id="frm_crearnegocio" class="form-vertical mt-1">
          <div class="form-group row">
            <div class="col-lg-5">
              <label class="col-form-label title-busq">Nombre de la ocupación</label>
              <input id="ocu_nombre" name="ocu_nombre" type="text" class="form-control input-rounded data-not-lt-active" minlength="2" placeholder="Nombre" required oninput="handleInput(event)"/>
            </div>
...
<div class="modal fade" id="editar_negocio-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="" id="exampleModalLabel">Editar ocupación</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form id="frm_editarnegocio" class="form-vertical mt-1">
          <div class="form-group row">
            <input id="ocu_ideditar" name="ocu_ideditar" type="hidden" required/>
            <div class="col-lg-5">
              <label class="col-form-label title-busq">Nombre de la ocupación</label>
              <input id="ocu_nombreeditar" name="ocu_nombreeditar" type="text" class="form-control input-rounded data-not-lt-active" placeholder="Nombre" required oninput="handleInput(event)"/>
            </div>
            <div class="row col-lg-12">
              <div class="col-sm-6 col-md-6 text-center mt-5">
              </div>

...
class ConfiguracionController extends ControllerBase
{
    ...
    {
        $this->tag->setTitle('Configuracion');
        parent::initialize();
    ...
    public function apariencia_indexAction()
    {
        ...
        $data_bit = [
            'bit_descripcion'=>"Ingresó a módulo de de configuracion de apariencia",
            'usu_id'=>$auth['id'],
            'bit_tablaid'=>0,
        ...
        if($this->request->isAjax()){
            ...
            $respuesta_modelo= $cof->actualizar_elementos_apariencia($data);
                ...
                if($respuesta_modelo['estado']==2)
                {
                    $auth = $this->session->get('auth');                    
                    $data_bit = [
                        'bit_descripcion'=>'Actualizó elementos gráficos del sistema',
                        'usu_id'=>$auth['id'],
                        'bit_tablaid'=>0,
                        'bit_modulo'=>"Configuración",
                        'bit_accion'=>2
    ...
    public function obtener_linkAction(){
        ...
        $auth = $this->session->get('auth');
        if(!$rol->verificar(79,$auth['rol_id'])) //el número en la funcion es el correspondiente a la bdd
          $this->response->redirect('errors/errorpermiso');
    ...
    public function correos_indexAction(){
        ...
        $data_bit = [
            'bit_descripcion'=>"Ingresó a módulo de de configuracion de envio de correos del sistema",
            'usu_id'=>$auth['id'],
            'bit_tablaid'=>0,
            'bit_modulo'=>"Configuración",
            'bit_accion'=>4

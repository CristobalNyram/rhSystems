...
class UsuarioController extends ControllerBase
{
            	...
            	{    
	                if($data['password1']==$data['password2']){
	                    ...
	                    if ($user->save() == false){                        
                            ...
                            $auth = $this->session->get('auth');
                            $data_bit = [
                                'bit_descripcion'=> 'Cambió su contraseña el usuario '.$user->usu_nombre.' '.$user->usu_primerapellido.' con ID  interno del sistema '.$user->usu_id,
                                'usu_id'=>$auth['id'],
                                'bit_tablaid'=>$user->usu_id,
                                'bit_modulo'=>"Usuario",
                                'bit_accion'=>2
                            ];
                    ...
                    if($data['password1']==$data['password2']){
                        ...
                        if ($user->save() == false){                        
                            ...
                            $auth = $this->session->get('auth');
                             ...
                             $data_bit = [
                                'bit_descripcion'=>'Se le actualizo su contraseña al usuario con nombre '. $user->usu_nombre.' '.$user->usu_primerapellido.' '.$user->usu_segundoapellido.' con Id interno del sistema '.$user->usu_id,
                                'usu_id'=>$auth['id'],
                                'bit_tablaid'=>$user->usu_id,
                                'bit_modulo'=>"Usuario",
                                'bit_accion'=>2,
                    ...
                    $mensaje_bitacora= 'Creó al usuario '. $data['usu_nombre']." ". $data['usu_primerapellido']." ". $data['usu_segundoapellido'].'con honorarios';
                        ...
                        { 
                            ...
                            $data_bit=[
                                ...
                                'usu_id'=>$auth['id'],
                                'bit_tablaid'=>$res_crear_usuario['usu_id'],
                                'bit_modulo'=>'Usuario',
                                'bit_accion'=>1,
                            ];
                            $this->bitacora_registro($data_bit,$auth);
                    ...
                    if($usuario_existe_rfc){
                        ...
                        if($respuesta_modelo['estado']===2)
                        {
                            $auth = $this->session->get('auth');
                             ...
                             $data_bit=[
                                'bit_descripcion'=>"Editó el usuario: ".$data["usu_nombreeditar"].' '.$data["usu_primerapellidoeditar"].' '.$data["usu_segundoapellidoeditar"].', folio interno: '.$clave,
                                'usu_id'=>$auth['id'],
                                'bit_tablaid'=>$clave,
                                'bit_modulo'=>"Trabajador",
                                'vac_id'=>0,
                                'bit_accion'=>2,
                            ];
    ...
    public function ajax_getanalista_excluir_un_analistaAction($ese_id){
        ...
        ->addFrom('Usuario','u')
        ->join('Relrolmenu','r.rol_id=u.rol_id','r')
        ->where('usu_estatus=2 and r.men_id=12 and rrm_estatus=1 and  usu_id <> '.$estudio->inv_id)
        ->getQuery()
        ->execute();
        if ($subs) {
            $result = $subs->toArray();
    ...
    public function ajax_get_all_ejecutivosAction()
    {
        $answer[0]=-1;
        try {
    ...
    public function ajax_usuario_auxiliaresAction(){
        $this->view->disable();
        ...
        ->columns('Usuario.usu_id, CONCAT(Usuario.usu_nombre, " ", Usuario.usu_primerapellido, " ", Usuario.usu_segundoapellido) as usu_nombre')
        ->leftjoin('Relrolmenu','r.rol_id=Usuario.rol_id','r')
        ->where('usu_estatus=2 and r.men_id=67 and rrm_estatus=1')
        ->execute();
        if ($subs) {
            $result = $subs->toArray();
        }

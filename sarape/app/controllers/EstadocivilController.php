...
class EstadocivilController extends ControllerBase
{
    ...
    {
        $this->tag->setTitle('Estado Civil');
        parent::initialize(); 
    ...
    public function indexAction()
    {
        ...
        if(!$rol->verificar(23,$auth['rol_id'])) //el número en la funcion es el correspondiente a la bdd
        {
    ...
    public function detallesAction($clave=0)
    {
        ...
        {
            $registro=Estadocivil::findFirstByesc_id($clave);
            if($registro)
            {
                $answer[0]=1;
                $answer[1]=$registro;
    ...
    public function ajax_estadoscivilAction()
    {
        $result = [];
        $subs = Estadocivil::find(array(
            "esc_estatus=2",
            'order'=>'esc_nombre',
            ));
        if ($subs) {
    ...
    public function nuevoAction()
    {
        ...
        if($this->request->isAjax())
        {
            ...
            $registro->esc_nombre= $data['esc_nombre'];
            $registro->esc_estatus=2;
            ...
            if($registro->save())
            {
                $auth = $this->session->get('auth');
                $data_bit = [
                    'bit_descripcion'=>'Creó un estado civil: '.$data['esc_nombre'],
                    'usu_id'=>$auth['id'],
                    'bit_tablaid'=>$registro->esc_id,
                    'bit_modulo'=>"Estado Civil",
                    'bit_accion'=>1
                ];
    ...
    public function editarAction($clave=0)
    {
        ...
        if($this->request->isAjax()&&$clave>0)
        {
            ...
            if($registro)
            {
                $registro->esc_nombre=$data["esc_nombreeditar"];
                ...
                if($registro->save())
                {
                    $auth = $this->session->get('auth');
                    $data_bit = [
                        'bit_descripcion'=>"Editó estado civil con id interno: ".$data["esc_ideditar"].',con nombre '.$registro->esc_nombre,
                        'usu_id'=>$auth['id'],
                        'bit_tablaid'=>$clave,
                        'bit_modulo'=>"Estado Civil",
                        'bit_accion'=>2
    ...
    public function eliminarAction($reg_id=0)
    {
        ...
        {
            $registro=Estadocivil::findFirstByesc_id($reg_id);
            $auth = $this->session->get('auth');
            ...
            if($registro)
            {
                ...
                {
                    $data_bit = [
                        'bit_descripcion'=>'Eliminó un estado civil con ID interno: '. $registro->esc_id,
                        'usu_id'=>$auth['id'],
                        'bit_tablaid'=>$registro->esc_id,
                        'bit_modulo'=>"Estado Civil",
                        'bit_accion'=>3

...
class GeneracionController extends ControllerBase
{
    public function initialize()
    {
        $this->tag->setTitle('Generación');
        parent::initialize();
    ...
    public function detallesAction($clave=0)
    {
        ...
        {
            $registro=Generacion::findFirstBygen_id($clave);
            if($registro)
            {
                $answer[0]=1;
                $answer[1]=$registro;
    ...
    public function ajax_generacionesAction()
    {
        $result = [];
        $subs = Generacion::find(array(
            "gen_estatus=2",
            'order'=>'gen_nombre',
            ));
        if ($subs) {
    ...
    public function nuevoAction()
    {
        ...
        if($this->request->isAjax())
        {
            ...
            $registro = new Generacion();
            $registro->gen_nombre= $data['gen_nombre'];
            $registro->gen_estatus=2;
            ...
            if($registro->save())
            {
                $auth = $this->session->get('auth');
                $data_bit = [
                    'bit_descripcion'=>'Creó una generación de empleo: '.$data['gen_nombre'],
                    'usu_id'=>$auth['id'],
                    'bit_tablaid'=>$registro->gen_id,
                    'bit_modulo'=>"Generación de empleo",
                    'bit_accion'=>1
                ];
    ...
    public function editarAction($clave=0)
    {
        ...
        if($this->request->isAjax()&&$clave>0)
        {
            ...
            if($registro)
            {
                $registro->gen_nombre=$data["gen_nombreeditar"];
                ...
                if($registro->save())
                {
                    $auth = $this->session->get('auth');
                    $data_bit = [
                        'bit_descripcion'=>"Editó la generación con id interno: ".$data["gen_ideditar"].',con nombre '.$registro->gen_nombre,
                        'usu_id'=>$auth['id'],
                        'bit_tablaid'=>$clave,
                        'bit_modulo'=>"Generación de empleo",
                        'bit_accion'=>2
    ...
    public function eliminarAction($reg_id=0)
    {
        ...
        {
            $registro=Generacion::findFirstBygen_id($reg_id);
            $auth = $this->session->get('auth');
            ...
            if($registro)
            {
                ...
                {
                    $data_bit = [
                        'bit_descripcion'=>'Eliminó la generación con ID interno: '. $registro->gen_id,
                        'usu_id'=>$auth['id'],
                        'bit_tablaid'=>$registro->gen_id,
                        'bit_modulo'=>"Generación de empleo",
                        'bit_accion'=>3

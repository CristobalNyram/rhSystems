...
class TipovacanteController extends ControllerBase
{
    public function initialize()
    {
        $this->tag->setTitle('Tipo de vacante');
        parent::initialize();
    ...
    public function detallesAction($clave=0)
    {
        ...
        {
            $registro=Tipovacante::findFirstBytip_id($clave);
            if($registro)
            {
                $answer[0]=1;
                $answer[1]=$registro;
    ...
    public function ajax_tipovacantesAction()
    {
        $result = [];
        $subs = Tipovacante::find(array(
            "tip_estatus=2",
            'order'=>'tip_nombre',
            ));
        if ($subs) {
    ...
    public function nuevoAction()
    {
        ...
        if($this->request->isAjax())
        {
            ...
            $registro = new Tipovacante();
            $registro->tip_nombre= $data['tip_nombre'];
            $registro->tip_estatus=2;
            ...
            if($registro->save())
            {
                $auth = $this->session->get('auth');
                $data_bit = [
                    'bit_descripcion'=>'Creó un tipo de vacante: '.$data['tip_nombre'],
                    'usu_id'=>$auth['id'],
                    'bit_tablaid'=>$registro->tip_id,
                    'bit_modulo'=>"Tipo vacante",
                    'bit_accion'=>1
                ];
    ...
    public function editarAction($clave=0)
    {
        ...
        if($this->request->isAjax()&&$clave>0)
        {
            ...
            if($registro)
            {
                $registro->tip_nombre=$data["tip_nombreeditar"];
                ...
                if($registro->save())
                {
                    $auth = $this->session->get('auth');
                    $data_bit = [
                        'bit_descripcion'=>"Editó la vacante con id interno: ".$data["tip_ideditar"].',con nombre '.$registro->tip_nombre,
                        'usu_id'=>$auth['id'],
                        'bit_tablaid'=>$clave,
                        'bit_modulo'=>"Tipo vacante",
                        'bit_accion'=>2
    ...
    public function eliminarAction($reg_id=0)
    {
        ...
        {
            $registro=Tipovacante::findFirstBytip_id($reg_id);
            $auth = $this->session->get('auth');
            ...
            if($registro)
            {
                ...
                {
                    $data_bit = [
                        'bit_descripcion'=>'Eliminó el tipo de vacante con ID interno: '. $registro->tip_id,
                        'usu_id'=>$auth['id'],
                        'bit_tablaid'=>$registro->tip_id,
                        'bit_modulo'=>"Tipo vacante",
                        'bit_accion'=>3
